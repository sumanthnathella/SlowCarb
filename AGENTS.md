...
- The Calendar tab is an undated Monday–Sunday view generated by `hooks.py` from the fixed-breakfast section and seven-day table in `docs/meals/starter-week.md`. Keep seven ordered four-column rows, use explicit meal names rather than 'repeat' references, and do not duplicate the schedule in calendar markup. Calendar's edit link deliberately points to the source plan. Keep shopping quantities manually synchronized.
- Tuesday dinner includes lemon-marinated carrot–radish salad and vegetarian, gluten-free kimchi as sides, without removing its egg/pulse protein. The refrigerated salad is not a shelf-stable pickle.
...
- Restaurant bowl suggestions live in `docs/meals/eating-out.md`. Distinguish ingredient choices from gluten cross-contact guarantees; use official US portion figures and never infer plain falafel allergens from a composed bowl's allergen label.
- Use local original SVG food illustrations in `docs/assets/illustrations/`, with descriptive alt text and explicit illustration labels. They are not restaurant photographs or portion measurements. Do not hotlink unlicensed food photography. Keep illustrations consistent with the textual plan.
